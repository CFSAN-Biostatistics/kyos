...
def parse_arguments(system_args):
    ...
    subparsers = parser.add_subparsers(
        dest="subparser_name", help=None, metavar="subcommand"
    )
    ...
    subparser.add_argument(
        dest="output_file", type=str, help="Output tabular data file."
    )
    ...
    subparser.add_argument(
        ...
        dest="truth_file",
        type=str,
        help="SNP Mutator summary file containing truth allele.",
        default=None,
    )
    subparser.add_argument(
        "-f",
        ...
        dest="tnfract",
        type=float,
        help="Fraction of True Negatives to write to the output file.",
        default=1.0,
    )
    subparser.add_argument(
        "-s",
        ...
        dest="rseed",
        type=int,
        help="Random seed to ensure reproducible results when using --tnfract.  Set to zero for non-deterministic results.",
        default=1,
    )
    subparser.add_argument(
        "--force_truth",
        dest="force_truth",
        action="store_true",
        help="Extract all known variant observations even if there is no coverage. Requires a summary file.",
        default=False,
    )
    ...
    subparser.add_argument(
        ...
        type=str,
        metavar="INFILE",
        help="Input tabulated feature files.",
        nargs="+",
    )
    subparser.set_defaults(func=merge_command)
    ...
    subparser.add_argument(
        ...
        type=str,
        metavar="FTR",
        help="Input tabulated feature file for training.",
    )
    subparser.add_argument(
        dest="validate_file_path",
        type=str,
        metavar="FTR",
        help="Input tabulated feature file for validation during training.",
    )
    subparser.add_argument(
        dest="model_file_path", type=str, metavar="MODEL", help="Output trained model."
    )
    subparser.add_argument(
        "--rseed",
        dest="rseed",
        type=int,
        help="Random seed to ensure reproducible results.  Set to non-zero value for reproducible results.",
        default=0,
    )
    ...
    subparser.add_argument(
        dest="model_file_path", type=str, metavar="MODEL", help="Input trained model."
    )
    subparser.add_argument(
        dest="test_file_path",
        type=str,
        metavar="FTR",
        help="Input tabulated feature file for testing.",
    )
    ...
    subparser.add_argument(
        ...
        type=str,
        metavar="FTR",
        help="Input tabulated feature file.",
    )
    subparser.add_argument(
        dest="vcf_file_path", type=str, metavar="VCF", help="Output VCF file."
    )
...
def train_command(args):
    ...
    """
    nn.train(
        args.train_file_path, args.validate_file_path, args.model_file_path, args.rseed
    )
...
def tabulate_command(args):
    ...
    if args.tnfract < 1.0 and args.truth_file is None:
        print(
            "You must specify a truth file when requesting a subset of the true negative observations.",
            file=sys.stderr,
        )
        exit(1)
    features.create_tabular_data(
        args.input_file,
        args.output_file,
        args.ref_file,
        args.truth_file,
...
def run_command_from_args(args):
    ...
    return args.func(
        args
    )  # this executes the function previously associated with the subparser with set_defaults
